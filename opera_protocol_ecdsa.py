#!/usr/bin/env python3
# opera_protocol_ecdsa.py - 完整的 Opera 协议实现
...
class Verifier:
    ...
    def request_token(self, owner: NetworkOwner, expiry_duration: int,
        ...
        if resp is None:
            return None
        # 解析响应：encrypted_token(变长) + owner_sig(64) + owner_cert(32) + nonce(32)
        # 由于加密长度不固定，这里采用简单方式：最后 64+32+32=128 字节是尾部
        tail = resp[-128:]
        ...
        if nonce_back != Nv:
            return None  # nonce 不匹配
        # 验证 owner 对 (Nv || apk || ids) 的签名
        try:
